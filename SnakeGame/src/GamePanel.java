...
public class GamePanel extends JPanel implements ActionListener{
	...
	final int x[] = new int[GAME_UNITS]; //x coordinates of snake
	...
	public void draw(Graphics g) {
		//turn the panel to matrix to easily see
		if(running) {
	...
	public void move() {
		for(int i = bodyParts; i > 0; i--) {
			x[i] = x[i-1];//shifting the coordinates by 1
			y[i] = y[i-1];
		}
		switch(direction) {
		case 'U':
	...
	public void actionPerformed(ActionEvent e) {
		...
		//if the game is running, move the snake
		if(running) {
			move();
			checkApple();
			checkCollisions();
